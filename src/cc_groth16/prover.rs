use crate::cc_groth16::{r1cs_to_qap::R1CSToQAP, Groth16, Proof, ProvingKey, VerifyingKey};
...
impl<E: Pairing, QAP: R1CSToQAP> Groth16<E, QAP> {
    ...
    pub fn create_proof_with_reduction_and_matrices(
        ...
        let prover_time = ark_std::start_timer!(|| "Groth16::Prover");
        let witness_map_time = ark_std::start_timer!(|| "R1CS to QAP witness map");
        ...
        ark_std::end_timer!(witness_map_time);
    ...
    fn create_proof_with_assignment(
        ...
        let c_acc_time = ark_std::start_timer!(|| "Compute C");
        ...
        // Compute A
        let a_acc_time = ark_std::start_timer!(|| "Compute A");
        ...
        ark_std::end_timer!(a_acc_time);

        // Compute B in G1 if needed
        let g1_b = if !r.is_zero() {
            let b_g1_acc_time = ark_std::start_timer!(|| "Compute B in G1");
            let s_g1 = pk.delta_g1.mul(s);
            let g1_b = Self::calculate_coeff(s_g1, &pk.b_g1_query, pk.beta_g1, &assignment);

            ark_std::end_timer!(b_g1_acc_time);
        ...
        // Compute B in G2
        let b_g2_acc_time = ark_std::start_timer!(|| "Compute B in G2");
        ...
        ark_std::end_timer!(b_g2_acc_time);

        let c_time = ark_std::start_timer!(|| "Finish C");
        let mut g_c = s_g_a;
        g_c += &r_g1_b;
        g_c -= &r_s_delta_g1;
        g_c += &l_aux_acc;
        g_c += &h_acc;
        ark_std::end_timer!(c_time);
