use crate::cc_groth16::{r1cs_to_qap::R1CSToQAP, Groth16, ProvingKey, VerifyingKey};
...
impl<E: Pairing, QAP: R1CSToQAP> Groth16<E, QAP> {
        ...
        C: ConstraintSynthesizer<E::ScalarField>,
    {
        ...
        let setup_time = ark_std::start_timer!(|| "Groth16::Generator");
        let cs = ConstraintSystem::new_ref();
        cs.set_optimization_goal(OptimizationGoal::Constraints);
        cs.set_mode(SynthesisMode::Setup);

        // Synthesize the circuit.
        let synthesis_time = ark_std::start_timer!(|| "Constraint synthesis");
        circuit.generate_constraints(cs.clone())?;
        ark_std::end_timer!(synthesis_time);

        let lc_time = ark_std::start_timer!(|| "Inlining LCs");
        cs.finalize();
        ark_std::end_timer!(lc_time);
        ...
        ///////////////////////////////////////////////////////////////////////////
        let domain_time = ark_std::start_timer!(|| "Constructing evaluation domain");

        let domain_size = cs.num_constraints() + cs.num_instance_variables();
        let domain = D::new(domain_size).ok_or(SynthesisError::PolynomialDegreeTooLarge)?;
        let t = domain.sample_element_outside_domain(rng);

        ark_std::end_timer!(domain_time);
        ///////////////////////////////////////////////////////////////////////////

        let reduction_time = ark_std::start_timer!(|| "R1CS to QAP Instance Map with Evaluation");
        let num_instance_variables = cs.num_instance_variables();
        let (a, b, c, zt, qap_num_variables, m_raw) =
            QAP::instance_map_with_evaluation::<E::ScalarField, D<E::ScalarField>>(cs, &t)?;
        ark_std::end_timer!(reduction_time);
        ...
        let g2_time = ark_std::start_timer!(|| "Compute G2 table");
        ...
        ark_std::end_timer!(g2_time);

        // Compute the B-query in G2
        let b_g2_time = ark_std::start_timer!(|| "Calculate B G2");
        let b_g2_query = FixedBase::msm::<E::G2>(scalar_bits, g2_window, &g2_table, &b);
        drop(g2_table);
        ark_std::end_timer!(b_g2_time);

        // Compute G window table
        let g1_window_time = ark_std::start_timer!(|| "Compute G1 window table");
        let g1_window =
        ...
        ark_std::end_timer!(g1_window_time);

        // Generate the R1CS proving key
        let proving_key_time = ark_std::start_timer!(|| "Generate the R1CS proving key");
        ...
        // Compute the A-query
        let a_time = ark_std::start_timer!(|| "Calculate A");
        let a_query = FixedBase::msm::<E::G1>(scalar_bits, g1_window, &g1_table, &a);
        drop(a);
        ark_std::end_timer!(a_time);

        // Compute the B-query in G1
        let b_g1_time = ark_std::start_timer!(|| "Calculate B G1");
        let b_g1_query = FixedBase::msm::<E::G1>(scalar_bits, g1_window, &g1_table, &b);
        drop(b);
        ark_std::end_timer!(b_g1_time);

        // Compute the H-query
        let h_time = ark_std::start_timer!(|| "Calculate H");
        ...
        ark_std::end_timer!(h_time);

        // Compute the L-query
        let l_time = ark_std::start_timer!(|| "Calculate L");
        let l_query = FixedBase::msm::<E::G1>(scalar_bits, g1_window, &g1_table, &l);
        drop(l);
        ark_std::end_timer!(l_time);

        ark_std::end_timer!(proving_key_time);

        // Generate R1CS verification key
        let verifying_key_time = ark_std::start_timer!(|| "Generate the R1CS verification key");
        ...
        ark_std::end_timer!(verifying_key_time);
        ...
        let batch_normalization_time = ark_std::start_timer!(|| "Convert proving key elements to affine");
        ...
        ark_std::end_timer!(batch_normalization_time);
        ark_std::end_timer!(setup_time);
