    ...
    <C as CurveGroup>::BaseField: PrimeField + Absorb,
{
    ...
    pub msg_ne: C::ScalarField,
    pub r_ne: C::ScalarField,
    pub r_elgamal: C::ScalarField,
    pub r_symmetric: C::BaseField,

    pub ck_id: Vec<C::BaseField>,
    pub pk_enc: Vec<C::BaseField>,
    ...
    <C as CurveGroup>::BaseField: PrimeField + Absorb,
{
        ...
        S: Serializer,
    {
        let multi_values_tuple = vec![
            ("ck_id", self.ck_id.clone()),
            ("pk_enc", self.pk_enc.clone()),
            ("g_k", self.g_k.clone()),
        ...
        let single_values_tuple = vec![
            ...
            ("msg_ne", self.msg_ne.clone().to_string()),
            ("r_ne", self.r_ne.clone().to_string()),
                ...
                D: Deserializer<'de>,
            {
                ...
                impl<'de> Visitor<'de> for FieldVisitor {
                    ...
                    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                        formatter.write_str("`sk_id`, `ck_id`, `pk_enc`, `g_k`, `msg_ne`, `r_ne`, `r_elgamal`, `r_symmetric`, `tree_proof`, or `leaf_pos`")
                    }
                        ...
                        E: de::Error,
                    {
                        match value {
                            "sk_id" => Ok(Field::SkId),
                            "ck_id" => Ok(Field::CkId),
                            "pk_enc" => Ok(Field::PkEnc),
                            "g_k" => Ok(Field::Gk),
                            "msg_ne" => Ok(Field::MsgNe),
                            "r_ne" => Ok(Field::RNe),
            ...
            <C as CurveGroup>::BaseField: PrimeField + Absorb,
        {
                ...
                V: MapAccess<'de>,
            {
                let mut multi_values_map: HashMap<Field, (&str, Option<Vec<C::BaseField>>)> =
                    HashMap::from([
                        ((Field::CkId, ("ck_id", None))),
                        ((Field::PkEnc, ("pk_enc", None))),
                        ((Field::Gk, ("g_k", None))),
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        // handle multi values
                        Field::CkId | Field::PkEnc | Field::Gk | Field::TreeProof => {
                        ...
                        Field::MsgNe => {
                            if msg_ne.is_some() {
                                return Err(de::Error::duplicate_field("msg_ne"));
                            }
                            let s: String = map.next_value()?;
                            msg_ne = Some(C::ScalarField::from(
                                BigUint::parse_bytes(s.as_bytes(), 16).unwrap(),
                            ));
                        }
                        Field::RNe => {
                            if r_ne.is_some() {
                                return Err(de::Error::duplicate_field("r_ne"));
                            }
                            let s: String = map.next_value()?;
                            r_ne = Some(C::ScalarField::from(
                                BigUint::parse_bytes(s.as_bytes(), 16).unwrap(),
                            ));
                        }
                        Field::RElgamal => {
                ...
                let msg_ne = msg_ne.ok_or_else(|| de::Error::missing_field("msg_ne"))?;
                let r_ne = r_ne.ok_or_else(|| de::Error::missing_field("r_ne"))?;
                let r_symmetric =
                ...
                Ok(ZkVotingCircuitWitnesses {
                    ...
                    ck_id: unwrap_map(&multi_values_map, Field::CkId).clone(),
                    pk_enc: unwrap_map(&multi_values_map, Field::PkEnc).clone(),
