    ...
    <C as Group>::ScalarField: PrimeField,
{
    ...
    pub tree_index: u32,
    pub voting_key: <C as CurveGroup>::Affine, //elgamal::PublicKey<C>
    pub ck: Vec<<C as CurveGroup>::Affine>,
}
    ...
    <C as CurveGroup>::BaseField: PrimeField + Absorb,
{
        ...
        S: Serializer,
    {
        ...
        let ck_serialized: Vec<String> = self
            .ck
            .iter()
            .map(|key| serialize_to_hex_string(key).unwrap()) // PublicKey를 직렬화
            .collect();

        state.serialize_field("ck", &ck_serialized)?;
        ...
        D: Deserializer<'de>,
    {
            ...
            <C as CurveGroup>::BaseField: PrimeField + Absorb,
        {
                ...
                V: MapAccess<'de>,
            {
                        ...
                        Field::Ck => {
                            ck = Some(
                                map.next_value::<Vec<String>>()?
                                    .iter()
                                    .map(|s| deserialize_from_hex_string(s).unwrap())
                                    .collect(),
                            );
                        }
                        Field::Paths => {
                ...
                let ck = ck.ok_or_else(|| de::Error::missing_field("ck"))?;
