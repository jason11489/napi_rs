...
use super::structure::{ZkSbtCircuitConstants, ZkSbtCircuitInputs, ZkSbtCircuitStatement};

type F = ark_bn254::Fr;

#[no_mangle]
pub extern "C" fn run_prove_zksbt(raw_input: Buffer, raw_pk: Buffer) -> Buffer {
    ...
    let circuit_inputs: ZkSbtCircuitInputs<F> = serde_json::from_str(&serialized_input).unwrap();
    let constants = get_constants().unwrap();
    let circuit: ZkSbtCircuit<F> = circuit_inputs.create_circuit(constants).unwrap();
...
#[no_mangle]
pub extern "C" fn run_verify_zksbt(raw_image: Buffer, raw_vk: Buffer, raw_proof: Buffer) -> bool {
