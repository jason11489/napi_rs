...
use super::structure::{ZkWalletCircuitConstants, ZkWalletCircuitInputs, ZkWalletCircuitStatement};

type C = EdwardsProjective;
type GG = EdwardsVar;

#[no_mangle]
pub extern "C" fn run_prove_zkwallet(raw_input: Buffer, raw_pk: Buffer) -> Buffer {
    ...
    let zkwallet_inputs: ZkWalletCircuitInputs<C> = serde_json::from_str(&serialized_input).unwrap();
    let constants = get_constants_zkwallet().unwrap();
    let circuit: ZkWalletCircuit<C, GG> = zkwallet_inputs
...
#[no_mangle]
pub extern "C" fn run_verify_with_processed_vk_zkwallet(
    raw_image: Buffer,
    ...
    let image: ZkWalletCircuitStatement<C> = serde_json::from_str(&serialized_image).unwrap();
...
#[no_mangle]
pub extern "C" fn run_verify_zkwallet(raw_image: Buffer, raw_vk: Buffer, raw_proof: Buffer) -> bool {
    let serialized_image = buffer::str_from_buffer(&raw_image);
...
pub fn get_constants_zkwallet() -> Result<ZkWalletCircuitConstants<C>, Error> {
