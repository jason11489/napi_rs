    ...
    D: Deserializer<'de>,
  {
    ...
    impl<'de> Visitor<'de> for VerifyingKeyWrapperVisitor {
        ...
        V: MapAccess<'de>,
      {
            ...
            Field::Gamma_abc_g1 => {
              ...
              gamma_abc_g1 = Some(
                v.chunks(2)
                  .map(|chunk| {
                    G1Affine::new(
                      Fq::from(BigUint::parse_bytes(chunk[0].as_bytes(), 16).unwrap()),
                      Fq::from(BigUint::parse_bytes(chunk[1].as_bytes(), 16).unwrap()),
                    )
                  })
                  .collect(),
              );
        ...
        let gamma_abc_g1 = gamma_abc_g1.ok_or_else(|| de::Error::missing_field("Gamma_abc_g1"))?;
