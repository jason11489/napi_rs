...
mod tests {
  ...
  fn test_semmetic_encryption_gadget() {
    ...
    let key_var = <MyGadget as SymmetricEncryptionGadget<MyEnc, Fr>>::SymmetricKeyVar::new_witness(
      ark_relations::ns!(cs, "gadget_randomness"),
      || Ok(&key),
    )
    .unwrap();

    let msg_var = <MyGadget as SymmetricEncryptionGadget<MyEnc, Fr>>::PlaintextVar::new_witness(
      ark_relations::ns!(cs, "gadget_randomness"),
      || Ok(&msg),
    )
    .unwrap();
