...
use crate::gadget::nullifiable_encryptions::elgamalnenc::{
  Ciphertext, ElGamalNEnc, MPublicKey, Parameters, Plaintext, PublicKey, Randomness,
};
...
mod test {
  ...
  use crate::gadget::nullifiable_encryptions::elgamalnenc::{
    constraints::NEncGadget, ElGamalNEnc, Plaintext, Randomness,
  };
  ...
  fn test_ne_gadget() {
    ...
    let parameters_var =
      ...
      .unwrap();
    let msg_var = <MyGadget as NullifiableEncryptionGadget<MyEnc, Fq>>::PlaintextVar::new_witness(
      ark_relations::ns!(cs, "gadget_message"),
      || Ok(&msg),
    )
    .unwrap();
    let pk_var = <MyGadget as NullifiableEncryptionGadget<MyEnc, Fq>>::PublicKeyVar::new_witness(
      ark_relations::ns!(cs, "gadget_public_key"),
      || Ok(&pk),
    )
    .unwrap();
    let mpk_var = <MyGadget as NullifiableEncryptionGadget<MyEnc, Fq>>::MPublicKeyVar::new_witness(
    ...
    let expected_var = <MyGadget as NullifiableEncryptionGadget<MyEnc, Fq>>::OutputVar::new_input(
